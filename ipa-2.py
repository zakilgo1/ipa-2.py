...
def vigenere_cipher(message, key):
    ...
    for z in message_list:
        if len(message_list) >= message_count:
            if key_count <= len(key_list):
                if z == ' ':
                    cipher_message = cipher_message + ' '
                else:
                    add_letter = ord(z) + ord(key_list[key_count - 1]) - 65
                    while add_letter > 90:
                        add_letter -= 26
                    cipher_message = cipher_message + chr(add_letter)
                key_count += 1
                message_count += 1
            else: 
                key_count = 1
...
def scytale_cipher(message, shift):
    ...
    ciphered_message = ""
    while (len(edited_message) % shift) != 0:
            edited_message = edited_message + "_"
    for x in edited_message:
        message_list.append(x)
    i = 0
